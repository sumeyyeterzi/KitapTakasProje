...
<body>
    ...
    if ($_GET) {
        ...
        $login = $db->prepare("SELECT * FROM sohbetettiklerim WHERE kimle=? AND kim=? OR  kimle=? AND kim=?");
        $login->execute(array($_SESSION["id"], $_SESSION["gonderen"], $_SESSION["gonderen"], $_SESSION["id"]));

        if ($login->rowCount()) {
        } else {
            $sorgu = $db->prepare("INSERT INTO sohbetettiklerim(kimle, kim) VALUES(?, ?)");
            $sorgu->bindParam(1, $_SESSION['id'], PDO::PARAM_INT);
            $sorgu->bindParam(2, $_SESSION['gonderen'], PDO::PARAM_INT);
            $sorgu->execute();

            $sorgu = $db->prepare("INSERT INTO sohbetettiklerim(kimle, kim) VALUES(?, ?)");
            $sorgu->bindParam(1, $_SESSION['gonderen'], PDO::PARAM_INT);
            $sorgu->bindParam(2, $_SESSION['id'], PDO::PARAM_INT);
            $sorgu->execute();
        }
    ...
    $login = $db->prepare("SELECT * FROM kitap WHERE Id=?");
    $login->execute(array($kitapID));
    ...
    <div class="container" style="margin-top: 50px;">
        <div class="row">
            ...
            <div class="col">
                <?php echo '<img src="./yuklenen/' . $user['resim'] . '" alt="">'; ?>
            </div>
